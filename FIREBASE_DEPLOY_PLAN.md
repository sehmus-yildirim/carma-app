...
keine Deploy-Freigabe. In diesem Aktualisierungsschritt wurden keine Tests,
Builds, Deploys oder Console-Aenderungen ausgefuehrt.

- **Live bestaetigt**: Name und grundlegender Ressourcentyp wurden am
  2026-08-22 direkt mit der Firebase-CLI oder per HTTPS geprueft.
...
## Nachweise vom 2026-08-22

| Bereich | Nachweis | Ergebnis |
|---|---|---|
| Git | `git status`, Branch und Upstream read-only | Branch `codex/android-package-de-plaqa-app`, vor Dokumentaenderung sauber, ein lokaler Commit vor `origin` |
...
| Functions | `firebase functions:list` | 12 Live-Functions, alle v2 / Node.js 22 / `europe-west3`; 11 lokale Exports fehlen live |
...
| Auth/App Check | lokaler Code und oeffentliche Action-Domain | E-Mail/Passwort, Google und SMS-MFA sind lokal implementiert; App Check ist build-gesteuert vorbereitet; Provider-, Template-, MFA- und Enforcement-Status muessen in der Console erneut geprueft werden |

## Letzter dokumentierter lokaler Pruefstand

Diese Pruefungen wurden in diesem Schritt nicht erneut ausgefuehrt:

- 31 Flutter-Testdateien, 169 Tests: bestanden.
...
| Ressource | Typ / lokaler Trigger | Quelle / lokaler Test | Live-Status 2026-08-22 | Blaze | Console / App Check | Gruppe |
|---|---|---|---|---|---|---|
| `syncProfilePhotoReferences` | Firestore Update `public_profiles/{userId}` | `profile_photo_sync.js` / Test vorhanden | Name live, CLI meldet jedoch `https`; Triggerabweichung klaeren | Ja | kein App-Check-Token bei Event-Trigger | F1 |
| `syncProfileVisibilityReferences` | Firestore Write `users/{userId}/settings/visibility` | `profile_vehicle_management.js` / Test vorhanden | nur lokal | Ja | kein App-Check-Token bei Event-Trigger | F1 |
| `searchPlate` | Callable | `plate_search.js` / Test vorhanden | live, Versionsgleichheit offen | Ja | App Check vor Erzwingung testen | F2 |
| `recordProfileView` | Callable | `index.js` / kein eigener Function-Test | nur lokal | Ja | App Check vor Erzwingung testen | F3 |
| `submitPlateHint` | Callable | `report_submission.js` / Rules-Tests, kein eigener Function-Test | nur lokal | Ja | Storage/Rules und App Check abgleichen | F3 |
...
| `submitProfileVerification` | Callable | `profile_verification.js` / Test vorhanden | nur lokal | Ja | Admin-/Dokumentenprozess und App Check | F5 |
| `reviewProfileVerification` | Callable | `profile_verification.js` / Test vorhanden | nur lokal | Ja | vertrauenswuerdiger Admin-Claim erforderlich | F5 |
| `saveProfileVehicle` | Callable | `profile_vehicle_management.js` / Test vorhanden | nur lokal | Ja | App Check vor Erzwingung testen | F6 |
| `setPrimaryProfileVehicle` | Callable | `profile_vehicle_management.js` / Test vorhanden | nur lokal | Ja | App Check vor Erzwingung testen | F6 |
| `deactivateProfileVehicle` | Callable | `profile_vehicle_management.js` / Test vorhanden | nur lokal | Ja | App Check vor Erzwingung testen | F6 |
| `updatePrimaryVehicleLocation` | Callable | `profile_vehicle_management.js` / Test vorhanden | nur lokal | Ja | Standort- und App-Check-Konfiguration | F6 |
...
| `maintainPlateHints` | Scheduler, alle 60 Minuten UTC | `report_cleanup.js` / Rules-Tests, kein eigener Function-Test | nur lokal | Ja | Cloud Scheduler/API und Storage | F3 |
| `cleanupProfileVerificationDocuments` | Scheduler, taeglich 04:30 Europe/Berlin | `profile_verification.js` / Test vorhanden | nur lokal | Ja | Cloud Scheduler/API; Dokumentenfreigabe haengt davon ab | F5 |
...
- **Profilbild- und Anzeigenamen-Synchronisierung**: F1. Lokal in derselben
  Function umgesetzt; der Live-Trigger ist widerspruechlich und daher nicht als
  inhaltlich aktuell zu behandeln.
- **Dokumenten-Ablauferinnerungen 30/14/3 Tage**: Bestandteil von F5 und durch
  `cleanupProfileVerificationDocuments` angestossen; live noch nicht aktiv.
...
- **Sicherheits- und Kinderschutzmeldungen**: Firestore-/Storage-Rules,
  Supportpfade sowie F3 (`submitPlateHint`/Cleanup). Der Callable-/Cleanup-Teil
  ist nur lokal.

## Deploy-Gruppen und Rueckfallplan

Die Befehle sind Dokumentation. Sie wurden nicht ausgefuehrt. Bereits live
gelistete Functions werden nur bei nachgewiesenem Quellunterschied aktualisiert.

| Gruppe | Geplanter Befehl | Abhaengigkeiten | Anschliessender Live-Test | Risiko / Rueckfall |
|---|---|---|---|---|
| F1 Profilreferenzen | `firebase deploy --project carma-a84e4 --only functions:syncProfilePhotoReferences,functions:syncProfileVisibilityReferences` | Blaze, Firestore API, aktuellen Live-Trigger von `syncProfilePhotoReferences` klaeren | Profilbild, Anzeigename und Sichtbarkeit mit zwei Testkonten aendern; Posts, Likes, Kommentare, Chats, Storys und Fahrzeuge pruefen | Fan-out-Schreibfehler; Logs begrenzt pruefen, bei Regression vorherigen Git-Stand der Functions einzeln redeployen |
...
| App Check | `firebase_app_check`, build-gesteuerte Aktivierung; Release nutzt Play Integrity | keine neue Pruefung in diesem Schritt | Provider-/Metrik-/Enforcement-Status erneut in Console pruefen; Functions erzwingen nicht durchgaengig | Functions-Aktualisierung: Ja | Debug-/Release-App registrieren, Monitoring zuerst | kein CLI-Deploy; Provider aktivieren, Testtoken/Play Integrity live pruefen, erst danach pro Produkt erzwingen; bei legitimen Blockaden Enforcement sofort deaktivieren |
...
- Blaze/Billing und Budgetwarnungen sind vor Function-Deploys erneut zu
  bestaetigen.
- `syncProfilePhotoReferences` hat live einen widerspruechlichen Triggertyp.
...
- App Check ist nicht durchgaengig aktiv und darf erst nach Provider-/Geraetetest
  erzwungen werden.
- Mehrere Functions besitzen keinen eigenen Function-Test.
- Dokumentenverifizierung bleibt bis Cleanup-Deploy, Live-Test und externer
